document.addEventListener('DOMContentLoaded', async function () {
    ...
    try {
        const response = await axios.get('http://localhost:3100/api/student/all',{headers});
    ...
    addInterviewButton.addEventListener('click', async (e) => {
        ...
        try {
            const response = await axios.post('http://localhost:3100/api/company/add', formData,{headers});
            if (response.data.success) {
